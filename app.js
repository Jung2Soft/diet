// Elite Fit Calendar (modern UI/UX + full functionality)
...
const sheet = $('#statusPicker');
const noteModal = $('#noteModal'); const noteText = $('#noteText'); const noteTitle = $('#noteTitle');
...
function updateKPIs(){
  const all=readAll(); let done=0, fail=0, total=0;
  const end=new Date(planStart); end.setDate(end.getDate()+planWeeks*7-1);
  for(let d=new Date(planStart); d<=end; d.setDate(d.getDate()+1)){
    const k = toKey(d); if(all[k]?.status){ total++; if(all[k].status==='success') done++; else if(all[k].status==='fail') fail++; }
  }
  kpiDone.textContent = String(done); kpiFail.textContent = String(fail);
...
function openSheet(ev, date){
  sheet.classList.remove('hidden'); sheet.style.transform='translateY(0)';
  const onPick = (e)=>{
    const act = e.target?.dataset?.status; if(!act) return;
  ...
  sheet.addEventListener('click', onPick, {once:true});
...
$('#applyPlan').addEventListener('click', ()=>{
  ...
  saveSettings(); currentMonth = new Date(planStart); render();
});
...
$('#exportBtn').addEventListener('click', ()=>{
  ...
  const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='fitcal-backup.json'; a.click(); URL.revokeObjectURL(a.href);
});
$('#importFile').addEventListener('change', (e)=>{
  ...
  const r=new FileReader(); r.onload=()=>{
    try{
      ...
      const s=JSON.parse(localStorage.getItem('fitcal.settings')); planStart=new Date(s.start); planWeeks=s.weeks||24;
      startDateInput.valueAsDate=planStart; weeksInput.value=planWeeks; currentMonth=new Date(planStart); render();
      alert('복원 완료!');
    }catch(e){ alert('복원 실패: JSON 형식 오류'); }
...
function bindGlobal(){
  ...
  $('#closeHelp').addEventListener('click', ()=> helpDialog.classList.add('hidden'));
  document.addEventListener('keydown', (e)=>{ if(e.key==='Escape'){ helpDialog.classList.add('hidden'); closeNote(); closeSheet(); } });
}
function showMotivation(){
  const k = new Date().toISOString().slice(0,10);
  let seed=0; for(const ch of k) seed += ch.charCodeAt(0);
  const idx = seed % QUOTES.length;
  document.getElementById('motivationText').textContent = '💬 ' + QUOTES[idx];
}
function buildPlanDetails(){
  const el = planDetails; el.innerHTML='';
  const legend = document.createElement('div'); legend.className='legend';
  legend.innerHTML = `
    <span class="dot phase1"></span> 1개월차
    <span class="dot phase2" style="margin-left:10px"></span> 2~3개월차
    <span class="dot phase3" style="margin-left:10px"></span> 4개월차
    <span class="dot phase4" style="margin-left:10px"></span> 5개월차
    <span class="dot phase5" style="margin-left:10px"></span> 6개월차
  `;
  el.appendChild(legend);
  PHASES.forEach(ph=>{
    const sec=document.createElement('section'); sec.style.marginTop='10px';
    sec.innerHTML = `
      <h3><span class="dot ${ph.color}"></span> ${ph.name} (W${ph.weeks[0]}~${ph.weeks.slice(-1)[0]})</h3>
